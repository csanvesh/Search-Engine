...
public class Porter {
	 ...
	 * 
	 * System.out.println("started"); Porter stemmer = new Porter();
	 * ReadInputFiles readInputFiles = new ReadInputFiles();
	 * List<ReadInputDatatype> content = readInputFiles.loadData();
	 * //Set<String> tokens = readInputFiles.tokenizeContent(content);
	 * 
	 * Set<String> sortedTokens = new TreeSet<String>(Collator.getInstance());
	 * for (String stemWords : tokens) { if (!stemWords.isEmpty()) {
	 * sortedTokens.add(stemmer.stripAffixes(stemWords.trim())); } } //
	 * sortedTokens=readInputFiles.checkStopList(sortedTokens); BufferedWriter
	 * writer = new BufferedWriter(new FileWriter(new
	 * File("./src/parser_output.txt"))); int i = 0; for (String sortedTerms :
	 * sortedTokens) { writer.write(sortedTerms + " " + i++); writer.newLine();
	 * // System.out.println(sortedTerms + " " + i++); } // for
	 * (List<ReadInputDatatype> stringList : content) { for (ReadInputDatatype
	 * fileContent : content) { writer.write(fileContent.getmDocId() + " " +
	 * fileContent.getIndex()); writer.newLine(); //
	 * System.out.println(fileContent.getmDocId() + " " + //
	 * fileContent.getIndex()); // } } writer.flush(); writer.close();
	 * 
	 * System.out.println("Completed"); // String stem =
	 * stemmer.stripAffixes(word); // System.out.println(stem); }
	 */
